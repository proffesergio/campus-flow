...
export default function StudentFeesPage() {
  ...
  const STATUS_CONFIG = {
    paid: { icon: CheckCircle, color: 'text-green-400', bg: 'bg-green-500/10 border-green-500/30' },
    ...
    partial: { icon: Clock, color: 'text-blue-400', bg: 'bg-blue-500/10 border-blue-500/30' },
    waived: { icon: CheckCircle, color: 'text-zinc-400', bg: 'bg-zinc-800 border-zinc-700' },
  };
    ...
    <div className="p-6 max-w-2xl mx-auto space-y-6">
      ...
      {pending.length > 0 && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-5 flex items-center justify-between">
          <div>
            <p className="text-sm text-red-300 font-medium">{t('fees.outstandingBalance')}</p>
            <p className="text-2xl font-bold text-white mt-1">{fmt(pendingTotal)}</p>
            <p className="text-xs text-zinc-400 mt-1">
              {pending.length === 1
                ? t('fees.invoiceDue_one', { count: pending.length })
                : t('fees.invoiceDue_other', { count: pending.length })}
            </p>
          </div>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-zinc-500 mt-1">
                        {t('fees.due')} {new Date(inv.dueDate).toLocaleDateString('en-BD', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </p>
                      {inv.status === 'partial' && (
                        <p className="text-xs text-blue-400 mt-0.5">
                          {t('fees.paidOf', { paid: fmt(inv.paidAmount), total: fmt(inv.amount) })}
                        </p>
                      )}
                      {inv.paidAt && inv.status === 'paid' && (
                        <p className="text-xs text-green-500 mt-0.5">
                          {t('fees.paidOn', { date: new Date(inv.paidAt).toLocaleDateString('en-BD', { day: 'numeric', month: 'short', year: 'numeric' }) })}
                        </p>
                      )}
                    </div>
                    <div className="text-right flex-shrink-0">
                      ...
                      <div className={`flex items-center gap-1 mt-1 justify-end ${cfg.color}`}>
                        ...
                        <span className="text-xs font-medium">{tStatus(inv.status)}</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
